...
> Geul은 한글에 초점을 둔 타이핑 애니메이션을 위한 라이브러리입니다.
...
> 여기서 포함된다는 것은 [Hangul](https://github.com/e-/Hangul.js/) 라이브러리에서 설명된 것과 같이, '두벌식 키보드를 기준으로 원문을 입력하기 위해 누르는 키들의 배열이 시작점 혹은 종점값을 입력할 때 누르는 키들의 배열을 포함하는 경우'를 의미합니다.
> <br><br>
> 예를 들어, `헬로우`라는 단어를 입력하기 위한 키들의 배열은 다음과 같습니다.
> <br> `['ㅎ', 'ㅔ', 'ㄹ', 'ㄹ', 'ㅗ', 'ㅇ', 'ㅜ']` <br> `헬롱`이라는 단어를 입력하기 위한 키들의 배열은 다음과 같습니다.
> <br> `['ㅎ', 'ㅔ', 'ㄹ', 'ㄹ', 'ㅗ', 'ㅇ']` <br>
> 따라서, `헬롱`이라는 문자열은 `헬로우`라는 문자열에 포함된다고 볼 수 있습니다.
